...
function SearchPageContent() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return sorted;
  }, [query, sortBy, priceRange, selectedBrands, expressDelivery]);

  const maxPrice = Math.max(...allProducts.map(p => p.price), 300000);
        ...
        <div className="mb-4 sm:mb-6">
          <form onSubmit={handleSearch} className="relative max-w-3xl mx-auto">
            ...
            <input
              ...
              className="w-full bg-white text-gray-900 placeholder:text-gray-500 text-sm sm:text-base px-3 sm:px-4 py-2.5 sm:py-3 pl-10 sm:pl-12 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm"
            />
            <button
              type="submit"
              className="absolute right-2 top-1/2 -translate-y-1/2 bg-blue-600 text-white px-4 sm:px-6 py-1.5 sm:py-2 rounded-md hover:bg-blue-700 transition-colors text-sm sm:text-base font-semibold"
            >
              Search
            </button>
          </form>
