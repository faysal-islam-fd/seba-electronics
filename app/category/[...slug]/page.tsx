...
function CategoryPageContent() {
  ...
  const subcategoryPath = slugArray.slice(1).join('/') || '';
  ...
  const normalizedCategory = mainCategory.toLowerCase().trim();
  const category = categoryMap[normalizedCategory] || {
    name: mainCategory || 'All Products',
    icon: '📦',
  ...
  const filteredProducts = useMemo(() => {
    ...
    // Subcategory filter
    if (subcategoryPath) {
      const searchTerm = subcategoryPath.toLowerCase().replace(/-/g, ' ');
    ...
    return sorted;
  }, [category.products, subcategoryPath, sortBy, priceRange, selectedBrands, expressDelivery]);
        ...
        <div className="hidden md:block mb-4">
          <Breadcrumb 
            items={[
              ...
              { label: category.name, href: `/category/${mainCategory}` },
              ...(subcategoryPath ? [{ label: subcategoryPath.replace(/-/g, ' ') }] : []),
            ]}
          />
        </div>
